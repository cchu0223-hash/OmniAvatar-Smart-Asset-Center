...
| `uploading` | 上传进度条面板 | **当前实现保持可见**（原设计收起隐藏，待 PM 确认） |
| `parsing` | 解析进度环面板 | **当前实现保持可见**（原设计收起隐藏，待 PM 确认） |
| `editing` | 台词编辑区（语言选择 + 讲话人列表 + 算力预览 + 操作按钮） | **当前实现保持可见**（原设计收起隐藏，待 PM 确认） |
| `generating` | 生成提交成功提示（「任务已提交，可在『我的』查看进度」）+ 「新建任务」按钮 | **当前实现保持可见**（原设计收起隐藏，待 PM 确认） |
...
- 台词列表：
  ...
  - 每条台词支持直接编辑（`<textarea>`）
  - **不提供逐段试听入口**：台词编辑台不展示播放按钮，用户无法在编辑阶段试听单条台词（`SegmentItem` 中播放按钮已移除）
...
- 任务卡片字段：
  ...
  - 视频时长
  - 状态标签（成功 / 处理中 / 失败）
  - **提交时间**（`YYYY-MM-DD HH:mm`，显示在卡片右侧 meta 区）
  - **过期时间**（`YYYY-MM-DD HH:mm 过期`，紧跟提交时间下方）：草稿任务 **15 天**后过期，合成视频 **30 天**后过期；时间基于提交时间戳计算
    - 过期时间右侧展示 **`(i)`** 图标，点击弹出说明 tooltip（再次点击页面任意区域关闭）：
      - 合成视频：「合成视频过期后不支持下载和预览」
      - 草稿：「草稿过期后不支持继续编辑」
- 列表底部（有任务时）展示小字提示：**「任务记录支持保留近 3 年提交历史」**
...
- **台词编辑阶段**（`editing`）：
  ...
  - 台词列表：
    ...
    - 每条台词可直接编辑（`<textarea>`）
    - 进入编辑后不展示「保存 / 取消」按钮
    - 失焦（`onBlur`）自动保存；按 `Escape` 取消并恢复原文
    - 交互逻辑与对口型落地页台词编辑台保持一致
    - 台词过长警告（`⚠️` + 黄色边框高亮）
  - 底部固定栏：
    - 预计算力消耗展示
...
- **右键入口新增源语言选择步骤**：用户右键 V1 片段点「AI对口型」后，先在左侧面板选择当前视频语种（中文 / 英文），点击确认后再进入上传 → 解析流程。
- **台词编辑交互对齐落地页**：台词段落点击进入编辑，`onBlur` 自动保存，`Escape` 取消并恢复原文；不提供「保存 / 取消」按钮。
- **引导气泡**：
...
- [ ] **非 idle 阶段 Tab 区是否应收起**：当前代码在 `uploading / parsing / editing` 阶段 Tab 区均保持可见，与原设计说明不符，待 PM 确认最终行为
- [ ] **源语言选择：自动识别 vs 手动选择**：当前代码提供 `SourceLangSelector`（中文 / 英文手动 toggle），数据模型描述为自动识别；待 PM 确认是否保留手动选择入口
- [ ] **undo/redo 是否需恢复目标语言下拉状态**：当前实现仅恢复段落文本快照（`segments`），语言下拉（`selectedLang`）在撤回后不还原，存在视觉偏差；待确认是否需修复
...
### Hero · 台词验证与提交前阻断（`validateAllSegments`）

- **触发时机**：点击「保存草稿」或「合成视频」时先执行整段校验。
- **校验规则**：按段计算 `duration = end-start`，要求 `textLength ∈ [duration×1, duration×8]`（字符/秒）；任一段不满足即阻断提交。
- **加载反馈**：校验期间（当前 mock 10s）在语言区下方显示「当前文本判断中…」，并禁用「保存草稿 / 合成视频」按钮。
- **失败反馈**：显示内联错误文案 `第 N 段文本时长不匹配，请调整文本长度`，用户修改后可重新触发校验。
- **实现备注**：当前为前端延时 mock，正式环境替换为 TTS/时长校验接口。

### Hero · 首次编辑提示弹窗（`showEditTipModal`）

- **触发逻辑**：用户本次会话首次点击任一段落进入编辑时弹出（`hasShownEditTip` 内存标志，非 localStorage 持久化）。
- **文案**：`修改后文本与原文本长度尽量相近，否则无法送入合成。`
- **样式**：弹窗与遮罩支持浅色/深色主题差异化配色（随 `theme` 切换）。
- **关闭方式**：点击弹窗外遮罩关闭。

### Hero · 段落编辑自动保存

- 进入编辑后使用 `<textarea>` 直接改写段落文本，不提供额外「保存」按钮。
- **自动保存**：`onBlur` 时若文本有变化即提交并写入撤销栈。
- **取消编辑**：按 `Escape` 退出编辑并恢复修改前文本，不写入撤销栈。

### Hero · 源语言选择器（`SourceLangSelector`）

- 位置：Hero 上传卡片下方（idle 阶段可见）。
- 选项：中文 / 英文，手动切换。
- 说明：当前实现与「源语言自动识别」并存，最终以 PM 决策为准（见开放问题）。

### Hero · 上传 / 解析状态内联展示

- **`uploading` 阶段**：Hero 左列原地显示上传进度条（文件名 + 百分比 + 进度条 + 「取消上传」按钮），右侧 `DemoVideoCard` 保持可见。
- **`parsing` 阶段**：Hero 左列原地显示解析进度环（SVG 圆环 + 百分比 + 文件名 + 「取消解析 / 后台解析」按钮），右侧 `DemoVideoCard` 保持可见。
- **`editing` 阶段**：切换至完整 `EditorPane`（台词编辑面板），Hero 区隐藏。
- **设计意图**：避免上传 / 解析时立即切换至编辑面板，保持 Hero 区视觉连贯性；仅在真正进入编辑阶段时才展示完整编辑界面。
...
- **提交时间 & 过期时间**：卡片右侧 meta 区展示两行信息，格式均为 `YYYY-MM-DD HH:mm`；过期时间后缀「过期」，右侧附 `(i)` 图标，点击展示对应说明（合成视频 / 草稿各异）。组件：`ExpireInfoIcon`（`src/LipSyncPage.tsx`）。
- **列表尾部提示**：任务列表非空时，列表最底部居中展示灰色小字「任务记录支持保留近 3 年提交历史」。

### Toast（合成提交 / 保存草稿）— 待完整实现（P1）

**当前实现**：
- 点击「**合成视频**」并提交任务后：在「我的」Tab 内展示 **banner**（青色边框 + 勾图标 + 「任务已提交，正在处理中，请稍候查看进度」），4s 自动消失。
- 点击「**保存草稿**」后：无任何 toast/banner 反馈，仅切换至「我的」Tab。
...
| V5.7.2 | 2026-04-13 | **MineCard 过期信息**：卡片右侧 meta 区新增提交时间（`YYYY-MM-DD HH:mm`）与过期时间（草稿 15 天 / 合成视频 30 天），格式一致；过期时间右侧附 `(i)` 图标，点击展示 tooltip 说明（合成视频过期后不支持下载和预览；草稿过期后不支持继续编辑）。列表非空时底部新增「任务记录支持保留近 3 年提交历史」灰色提示。 |
| V5.7.2 | 2026-04-14 | **台词编辑台移除试听入口**：`SegmentItem` 中逐段播放按钮已移除，用户在台词编辑阶段不支持试听单条台词。 |
| V5.7.2 | 2026-04-24 | **PRD 与实现对齐**：新增「台词验证与提交前阻断」子节（`validateAllSegments`，1–8 字符/秒规则，10s 模拟延迟，内联错误展示）；新增「首次编辑提示弹窗」子节（一次性弹窗、日/夜主题适配）；新增「源语言选择器」子节（`SourceLangSelector`，当前为手动选择，待 PM 确认是否保留）；新增「段落编辑自动保存」子节（blur 提交、Escape 取消）；将 Toast 规格标注为"待完整实现 P1"并补充当前 banner 实现；修正状态机 Tab 可见性规则（当前始终可见，与原设计不符，已在开放问题中标注待确认）；新增三个开放问题（Tab 可见性、源语言策略、undo/redo 是否恢复 selectedLang）。 |
| V5.7.2 | 2026-04-27 | **专业剪辑 AI 对口型面板对齐落地页交互**：右键「AI对口型」后新增源语言选择步骤（中文 / 英文 toggle + 确认按钮），确认后进入上传 → 解析 → 编辑流程；台词编辑台移除「保存 / 取消」按钮，改为 `onBlur` 自动保存、`Escape` 取消，与落地页台词编辑台逻辑完全一致；移除 ±5% 长度校验阻断保存逻辑，仅在保存后文字超过 200 字符时显示警告。**落地页 Hero 区上传/解析状态内联展示**：`uploading` 和 `parsing` 阶段不再切换至 `EditorPane`，而是在 Hero 左列原地显示进度条 / 进度环，右侧 `DemoVideoCard` 保持可见；仅 `editing` 阶段切换至完整编辑面板。 |
