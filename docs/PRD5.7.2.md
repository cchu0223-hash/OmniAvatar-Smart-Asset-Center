...
- 目标语言选择：下拉选择，支持 8 种语言（ZH / EN / JA / RU / KO / AR / ES / YUE）
  - **切换语种后**：在语言控件**下方**展示 **「翻译中…」**状态（含加载指示与目标语种提示）；**翻译进行中**时语言下拉及列表项**不可再次切换**，避免并发请求。
  - 翻译完成后（前端 Mock 约 **1.4s**，正式环境以接口为准）：**自动用新语种台词整批覆盖**当前列表展示内容；若用户正在编辑某条台词，切换时应**退出编辑态**。
  - **撤回**：与台词编辑共用 **撤销栈**（实现参考 `segHistory` / 撤回按钮）；**撤回可撤销最近一次语言切换**（恢复切换前的整段台词快照），**重做**可恢复。
  - 正式环境：翻译结果由 **翻译 / 对齐服务**返回；当前 Demo 使用 `SEGMENTS_FOR_LANG` 本地 Mock。
- 台词列表：
...
## 落地页与首页：功能逻辑补充（与当前实现同步）

> 本节描述已在代码中落地的**交互与数据流**，便于评审与联调；实现主要位于 `src/LipSyncPage.tsx`、`src/index.css`，首页推荐区相关位于 `src/App.tsx`。

### Hero · 上传卡片（`UploadCard`）

- **热区**：整张上传卡片（含主文案区与底部「上传视频」条）点击均需触发同一上传逻辑；子元素通过样式避免截获指针事件，保证命中外层卡片。
- **事件**：`onClick` 须以 **无参回调**（如 `() => onUpload()`）调用上传处理函数；**不得**将 React 的合成事件对象当作首参传入，否则会被误判为「文件名」并导致运行异常。若上传函数签名支持文件名，仅在接受**字符串**时视为真实文件名。
- **拖放**：拖放完成仍走同一套上传入口（与点击行为一致）。

### Hero · 内嵌编辑器顶栏（`EditorPane` · `hero-editor-top`）

- **布局**：在 `editing` 阶段，「**AI 水印**」开关与标题紧邻：置于**文件名（`hero-editor-filename`）同一行右侧**，与文件名之间用竖向分隔区分；**预计消耗算力**与「取消合成 / 保存草稿 / 合成视频」仍保留在顶栏最右侧，避免主操作被挤压。
- **无障碍**：文件名溢出时 `text-overflow: ellipsis`，并可通过 `title` 查看全文。

### Hero · 台词区与分段（`SegmentItem`）

- **说话人圆标（如 S1）**：浅色主题（`data-theme="light"`）下圆标内文字为**白色**，以保证与渐变底对比度；深色主题下为**深色字**，与既有视觉一致。

### 「我的合成」任务卡片（`MineCard`）

- **下载**：成功态的下载入口为**仅图标按钮**（Material `download`），需提供 `aria-label` / `title`（如「下载」）。
- **重试**：`failed` 态「**重试**」与「继续编辑 / 重新编辑」一致，调用 **`onResume(task)`**，回到 **`editing`** 工作台；恢复台词时若任务上**无** `savedSegments`，则回退为默认分段数据并重建撤销栈，且**清除**未闭合的段落编辑态（如 `editingId`）。

### Toast（合成提交 / 保存草稿）

- **触发**：
  - 点击「**合成视频**」并提交任务后：展示 **「合成任务已提交」**，同时切换至「我的」Tab 与工作区 `idle`（与既有任务写入逻辑一致）。
  - 点击「**保存草稿**」后：展示 **「您的草稿已保存至「我的合成」，您可随时重新编辑。」**（标点与书名号以产品文案为准），并切换至「我的」Tab。
- **主题**：
  - **浅色模式**：浅色背景 + 深色正文 + 绿色勾图标，与日间界面协调。
  - **深色模式**：深色面板 + 浅色正文 + 绿色勾图标。
- **时长**：短文案约 **2.8s** 自动消失；较长文案（如保存草稿说明）约 **4.2s**，便于读完。
- **技术**：样式类 `lipsync-toast`，随 `document.documentElement` 的 `data-theme` 切换；依赖 `useTheme` 写入的主题属性。

### 首页 `/` · 推荐瀑布流（`App.tsx`）

- **类型角标**（视频 / 图片）：左上角标签文字与图标需保持**白色**等高对比度，避免在渐变底上发灰。
- **「做同款」**：卡片悬停层内按钮的图标与「做同款」文案均为**白色**（含显式 `color`，避免被全局样式覆盖）。

### 品牌主标题（`TypewriterTitle`）

- **「讯飞智作」（`.title-t1`）**：不再使用浅色渐变字；**深色 / 浅色主题均为深灰色实色字**（浅色页略深、夜间页为略浅的深灰以保证可读），与后续彩色渐变副标题（`，AI对口型…` 段）区分职责。
...
| V5.7.2 | 2026-04-02 | **落地页 Hero `EditorPane`**：目标语言切换后于下拉**下方**展示「翻译中…」；约 **1.4s** Mock 完成后**整批覆盖**台词；翻译中禁用语言切换与段落交互；**撤回 / 重做**可撤销或恢复本次语言切换（与片段编辑共用历史栈）。实现参考 `SEGMENTS_FOR_LANG`、`handleSelectLang`、`langTranslating`。 |
| V5.7.2 | 2026-04-03 | **交互与文案对齐实现**：上传整卡可点及点击事件入参约定；**保存草稿 / 合成视频** Toast 文案与浅深色主题样式及展示时长；**MineCard** 下载图标化、失败**重试**回编辑态与无草稿台词时的回退规则；**EditorPane** 顶栏 **AI 水印**与文件名并排；**SegmentItem** 说话人圆标浅色白字；首页推荐卡**类型角标 / 做同款**字色；**TypewriterTitle**「讯飞智作」深灰实色字。详见上文「落地页与首页：功能逻辑补充」。 |
