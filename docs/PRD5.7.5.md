...
7. 生成完成后，V1 轨道进度条展示「100% 生成完成」并持续显示（不自动消失）；对口型面板切换至结果预览态，展示生成视频（支持播放/暂停、下载、全屏），并提供「一键替换轨道」和「重新编辑」两个操作按钮；右键 V1 轨道片段可选择「查看生成任务」或「一键替换轨道」。
...
| `generating` | 「生成中…」进度条 + 百分比 + 提示文案「可切换到其他面板继续工作，完成后可右键点击 AI 对口型查看任务」 | **V1 轨道片段上叠加渐变进度条**（紫 → 青，宽度 = progress%） |
| `done` | 结果预览面板：顶部「生成完成 ✓」标识；16:9 视频播放区（支持播放/暂停、下载、全屏）；底部「一键替换轨道」主按钮 + 「重新编辑」次按钮 | V1 轨道片段叠加「100% 生成完成」蓝青渐变 overlay，持续展示直至用户执行替换 |

**入口**：
- **右键菜单 — 未生成时**（右键 V1 轨道视频片段，`stage ≠ done`）→ 菜单项「AI对口型」→ 左侧面板切换至对口型面板，直接使用轨道上视频的 `sourceUrl` 进入 `parsing` 态
- **右键菜单 — 生成完成后**（`stage = done`）→ 菜单项变为两项：
  - 「查看生成任务」→ 打开对口型面板并展示结果预览态（`done`）
  - 「一键替换轨道」→ 执行替换逻辑，清除 overlay，重置状态机至 `idle`
...
- **右键菜单入口**（主入口）：
  - 右键 V1 视频轨道上的片段时，上下文菜单新增「AI对口型」选项（强调色展示，与其他选项用分隔线区分）
  - 点击后：
    - 左侧面板自动切换至对口型面板
